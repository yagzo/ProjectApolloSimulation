...
def plot_singlebed(data, roi=None, pause=False,  out_place='./plot', cycles=None,text=''):

  if pause:
    #so we can stop and still get Enter input from user
    plt.ion()
  param=data.data1.param
  bunch=Obj()
  bunch.t=data.data1.t
  ...
  if cycles is None:
    ...
    mask=np.logical_and(bunch.t >= data.param.norm_tend*cycles, bunch.t <= data.param.norm_tend*(cycles+1))
    cycle_string='cycle {}'.format(cycles)
  elif len(cycles)==2:
    mask=np.logical_and(bunch.t >= data.param.norm_tend*cycles[0],bunch.t <= data.param.norm_tend*(cycles[1]+1))
    cycle_string='cycles {}-{}'.format(cycles[0],cycles[1])
  else:
    raise ValueError("plots(): bad cycles argument {}".format(cycles))
  #times that will be plotted
  ...
  if roi is not None:
    ...
    rng=[max(bunch.t)-param.norm_tend,max(bunch.t)]
  else:
    rng=[min(times),max(times)]   # do everything
  #if asking for more than we plotted, adjust
  if rng[1] > max(bunch.t):
    dist=rng[1]-rng[0]
    rng[0]=max(bunch.t)-dist
    rng[1]=max(bunch.t)
  q=np.logical_and(rng[0] <= bunch.t, bunch.t <= rng[1])
  ...
  for i,d in enumerate((data.data1,)):
    ...
    ax1.set_title('velocity - Container {} {}'.format(i+1,text),pad=3)
    ax1.set_ylabel('vel [normalized]')
    v_to_mps2=functools.partial(v_to_mps, norm_v0=d.param.norm_v0)
